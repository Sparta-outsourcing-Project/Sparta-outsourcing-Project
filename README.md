...
- 특히 api를 받아오는 과정에서, 원하는 데이터를 쓰기 위해 알아보는 과정이 생각보다 어려웠고, api 데이터를 받아오는 역할을 처음 맡다보니 데이터를 어느 정도까지 가공해서
  주는 게 좋을 지 판단하는 데에 있어 고민이 많기도 했습니다. 또한, api 명세서 등을 통해 줄 수 있는 데이터를 처음부터 명확히 하지 못해 팀원들에게 혼란을 줬던 것 같아 아쉽기도
  했으나, 소통의 중요성을 깨닫는 계기가 되었습니다.
  또한 react-query를 사용해 외부 DB와 UI를 동기화시키는 것이 익숙하지 않아 많은 에러에 부딪혔지만, 이를 통해 즐겨찾기 기능을 구현할 수 있어 뿌듯했습니다.
  그리고 적극적으로 서로의 문제를 해결해주면서 의견을 물어보는 팀원들에게서 배울 점이 많다고 느꼈습니다.
